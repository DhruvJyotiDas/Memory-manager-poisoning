...
def calibrate_heuristic(task: Task, seed: int = 0,
    ...
    best_tau, best_score = grid[-1], -1.0
    for tau in grid:
        m = HeuristicManager(tau=tau)
        store, _ = run_benign_stream(m, task, train=False, seed=seed)
        acc = benign_accuracy(store, task)
        admit_rate = len(store) / max(len(task.facts), 1)
        if 0.20 <= admit_rate <= 0.75:
            if acc > best_score:
                best_score, best_tau = acc, tau
    return HeuristicManager(tau=best_tau), best_tau, best_score
